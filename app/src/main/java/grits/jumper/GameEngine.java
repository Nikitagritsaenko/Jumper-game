...
public class GameEngine {
    ...
    GameEngine(Context context) {
        ...
        if (strLang.equalsIgnoreCase("english")) {
            size = AppConstants.getExitW() / 2.0f;
        }
        else {
            ...
            if (b.getType() == BlockType.END) {
                ...
                if (b.getDegree() == 0) {
                    if (b.getType() == BlockType.PORTAL) {
                        ...
                        if (b.isRestoring()) {
                            b.setAlpha(b.getAlpha() + AppConstants.INF_PORTAL_DISCOLORATION_SPEED);
                        } else {
                            b.setAlpha(b.getAlpha() - AppConstants.INF_PORTAL_DISCOLORATION_SPEED);
                        }
                    }
                ...
                if (b.getType() == BlockType.PORTAL_REUSABLE) {
                    if (alpha < 0) {
                        b.setRestoring(true);
                    } else if (alpha >= 255 && b.isRestoring()) {
                        b.setRestoring(false);
                        b.increaseDegree();
                    }
                }
            }
    ...
    void updateCollision() {
        if (player.getY() < 0) {
            ...
            if (AppConstants.getGravity() < 0) {
                AppConstants.invertGravity();
                AppConstants.getBitmapBank().flipPlayerVertical();
            }
            return;
        }
        ...
        for (Block b : blocks) {
            ...
            /* portals */
            if (b.getType().isPortal() && b.getDegree() > 0) {
            ...
            /* check if player land on block */
                ...
                } else if (b.getType() == BlockType.GRAVITY_REVERSER) {
                    soundPlayer.playGravityReverseSound();
                } else if (b.getType() == BlockType.LR_REVERSER) {
                    soundPlayer.playDirectionReverseSound();
                }
                else {
                ...
                if (b.getType() == BlockType.LR_REVERSER) {
                    player.swapDirection();
                }

                if (b.getType() != BlockType.DESTROYABLE_2) {
                    b.decreaseDegree();
                    if (b.getType() == BlockType.GRAVITY_REVERSER) {
                        AppConstants.invertGravity();
                        AppConstants.getBitmapBank().flipPlayerVertical();
                    }
                }
            ...
            } else if (sx <= 1 && sy < 0 && s < 0.7 * AppConstants.getPlayerH()) { /* check crush to ceil of block */
                if (b.getType() == BlockType.GRAVITY_REVERSER) {
                    hasTouchedBlock = true;
                    b.decreaseDegree();
                    AppConstants.invertGravity();
    ...
    void restartGame() {
        ...
        player.setLeftRightDirectionSwapped(false);
    ...
    ArrayList<Block> getBlocksAlive() {
        ...
        for (Block b: blocks) {
            if (b.getIdx() == lastBlockIdx || b.getIdx() == endIdx || b.getType().isPortal()) {
                continue;
            }
